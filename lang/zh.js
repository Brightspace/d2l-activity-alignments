/* eslint quotes: 0 */

export default {
	"a11yHeaderAriaLabel": "树级 {level} - {status} - {name}",
	"a11yLeafAriaLabel": "树叶 {shortCode} - {status} - {description}",
	"add": "添加",
	"addLabel": "添加选择",
	"alignmentRemoved": "已移除对齐",
	"cancel": "取消",
	"cancelLabel": "取消选择",
	"collapsed": "折叠",
	"defaultTagType": "标记",
	"directAlignments": "{headerTitle} 直接与此活动对齐",
	"error": "出现错误",
	"expanded": "展开",
	"indirectAlignments": "{headerTitle} 与量规标准对齐",
	"navigationDescription": "{title}：使用箭头键浏览 {typeName}，使用 Delete 或退格键移除 {typeName}。",
	"navigationDescriptionNoActions": "{title}：使用箭头键浏览 {typeName}。",
	"noSearchResultFor": "未找到“{searchText}”的结果",
	"notSelected": "未选择",
	"outcomesHierarchicalTree": "结果层次结构树",
	"removeAlignment": "移除对齐",
	"searchCleared": "搜索已清除",
	"searchOutcomes": "搜索结果",
	"searchPlaceholder": "搜索...",
	"searchResultFor": "“{searchText}”的 {numOfResults} 个搜索结果",
	"searchResultsNumber": "{numOfResults} 个搜索结果",
	"selected": "已选"
};
